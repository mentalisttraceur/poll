...
char const stdoutWritingError[] = "poll: error writing to stdout: \n";
...
\*/
int writeUntilError(int fd, void * buf, size_t count)
{
 do
 {
...
/*\
We try to write to stdout, if that fails we write the error to stderr. If that
fails, we really can't do anything else.
\*/
void writeOut(void * buf, size_t count)
{
 int err = writeUntilError(STDOUT, buf, count);
 if(err)
 {
  struct iovec errMsg[2];
  char * errStr = strerror(err);
  errMsg[0].iov_base = stdoutWritingError;
 ...
 *outputBufferPtr = '\n';
 ...
 writeOut(outputBuffer, outputBufferPtr + 1 - outputBuffer);
}
